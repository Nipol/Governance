    ...
    function cast(bytes32[] memory spells, bytes[] memory elements) internal returns (bytes[] memory) {
        ...
        for (uint256 i; i != length;) {
            ...
            if (flags & FLAG_CT_MASK == FLAG_CT_DELEGATECALL) {
                (success, outdata) =
                    address(uint160(uint256(command))).delegatecall(elements.toSpell(bytes4(command), indices));
            } else if (flags & FLAG_CT_MASK == FLAG_CT_CALL) {
                (success, outdata) = address(uint160(uint256(command))).call(elements.toSpell(bytes4(command), indices));
                ...
                (success, outdata) =
                    address(uint160(uint256(command))).staticcall(elements.toSpell(bytes4(command), indices));
            }
