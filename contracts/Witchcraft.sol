...
library Witchcraft {
    ...
    function toSpell(bytes[] memory elements, bytes4 selector, bytes32 indices)
        internal
        view
    ...
    function writeOutputs(bytes[] memory elements, bytes1 index, bytes memory output)
        internal
        pure
        returns (bytes[] memory newEle)
    {
    ...
    function writeTuple(bytes[] memory elements, bytes1 index, bytes memory output) internal view {
